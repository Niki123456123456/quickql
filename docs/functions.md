...
Returns the item in the first argument array at the zero-based integer index from the second argument. When the second argument is a two-item array like `[2, 4]`, returns a slice from the start index up to but excluding the end index. A one-item range like `[2]` returns a slice from that index through the end of the array. Returns `null` when the first argument is not an array, the index is invalid, or the index is out of range.

```ql
MAP first = AT(items, 0)
MAP second = AT(['a', 'b', 'c'], 1)  -- 'b'
MAP slice = AT(['a', 'b', 'c', 'd', 'e'], [2, 4])  -- ['c', 'd']
MAP tail = AT(['a', 'b', 'c', 'd', 'e'], [2])  -- ['c', 'd', 'e']
```
