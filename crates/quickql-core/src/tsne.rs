...
mod tests {
    ...
    #[test]
    fn tsne_value_returns_row_wise_embedding() {
        ...
        let embedding = tsne_value(Some(&data), Some(&config));
        let rows = embedding.as_array().expect("embedding should be an array");
    ...
    #[test]
    fn tsne_value_rejects_fft_without_feature() {
        let data = serde_json::json!([[0.0], [1.0]]);
        let config = serde_json::json!({ "approxType": "fft" });

        assert_eq!(tsne_value(Some(&data), Some(&config)), Value::Null);
    }
}
